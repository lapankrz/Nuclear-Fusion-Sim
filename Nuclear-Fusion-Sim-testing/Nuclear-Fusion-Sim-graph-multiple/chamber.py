...
class Chamber:
    def __init__(self, laser, x=1, y=1, z=1, scenario=1, particle_pairs=20):
        # chamber dimensions
        self.laser = laser
        self.x = float(x)
        self.y = float(y)
    ...
    def update_particles(self):
        ...
        for p in self.particles:
            ...
            #laser_state = 0
           # self.Temperature += self.LaserExcitation(self.dt, laser_state)
            #vsqr = sqrt((3 * C.k * self.Temperature) / p.mass_kg)
            #p.velocity += sqrt(vsqr ** (1. / 3))
            #if p.velocity.x * self.dt >= self.x / 2 or p.velocity.y * self.dt >= self.y / 2 or p.velocity.z * self.dt >= self.z / 2:
            #    self.dt = self.dt / 4
            #if p.velocity.x * self.dt <= self.x / 2 or p.velocity.y * self.dt <= self.y / 2 or p.velocity.z * self.dt <= self.z / 2:
            #    self.dt = self.dt * 4
        self.clip_to_bounds()
